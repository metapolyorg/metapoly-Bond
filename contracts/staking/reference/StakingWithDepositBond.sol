...
contract StakingWithDepositBond is Initializable, OwnableUpgradeable {
    ...
    function initialzeStaking(IERC20Upgradeable _D33D, IStakingToken _sD33D, address distributor_, address _stakingWarmUp, 
        ...
        uint _firstEpochTimestamp, address _DAO, address _usmMinter, address _vD33D, uint _warmupPeriod) external onlyOwner {
    ...
    function stake(uint _amount, address _receiver) external returns (bool) {
        return _stake(_amount, _receiver, _msgSender(), false);

    }

    function stakeFromBond(uint _amount, address _receiver) external returns (bool){
        address _sender = _msgSender();
        require(isBond[_sender], "Staking: Only Bond");
        return _stake(_amount, _receiver, _sender, true);
    }

    function _stake(uint _amount, address _receiver, address _sender, bool isBondDeposit) internal returns(bool) {
        rebase();
        ...
        warmupInfo[ _receiver ] = Claim ({
            ...
            expiry: isBondDeposit || info.expiry > block.timestamp ? info.expiry + warmupPeriod : 0
        });
    ...
    function claim(address _sender) internal returns (uint amount){
        ...
        require(info.expiry < block.timestamp, "Staking: locked" );
    ...
    function updateWarmupPeriod(uint _warmupPeriod) external onlyOwner{
        warmupPeriod = _warmupPeriod;
    }

    function updateBondAddress(address[] memory _bonds, bool[] memory _status) external onlyOwner {
        for(uint i=0; i< _bonds.length; i ++ ) {
            isBond[_bonds[i]] = _status[i];
        }
    }
