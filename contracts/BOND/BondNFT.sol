...
contract NFTBond is Initializable, IERC721ReceiverUpgradeable, ChainlinkClient {
    ...
    function initialize(address _D33D, address _principle, address _treasury, address _bondCalculator, 
        address _staking, address _DAO, address _admin, address _oracle, bytes32 memory _jobId, uint _oracleFee) external initializer {
    ...
    function setOracleParams(address _oracle, bytes32 memory _jobId, uint _oracleFee) external onlyOwner {
        oracle = _oracle;
        jobId = _jobId;
        oracleFee = _oracleFee ;
    }
