...
const unlockedAddress = "0x99B7831D541B090fF385399a33c191a8CE537Ece"
...
describe("USDT BOND", () => {
    it("Should Work", async () => {
        let { user1, user2, D33D, treasury, Bond, USDT, deployer } = await setup()
...
const setup = async () => {
    ...
    let Bond = await setupBond(D33D, treasury, deployer)//await ethers.getContractAt("BondContract", addresses.bond.USDTBond)
    ...
    await treasury.connect(deployer).toggle(2, USDT.address, ethers.constants.AddressZero)

    await USDT.connect(unlockedSigner).transfer(user1.address, ethers.utils.parseUnits("5", "6"))
    await USDT.connect(unlockedSigner).transfer(user2.address, ethers.utils.parseUnits("500", "6"))
    await USDT.connect(user1).approve(Bond.address, ethers.constants.MaxUint256)
    await USDT.connect(user2).approve(Bond.address, ethers.constants.MaxUint256)

    return { user1, user2, D33D, treasury, Bond, USDT, deployer }
}
...
const setupBond = async (d33d, treasury, deployer) => {
    ...
    let bondContract = await upgrades.deployProxy(BondContract, [d33d.address, addresses.tokens.usdt, treasury.address,
