...
const unlockedAddress = "0x436ff030ce062d5baEB3d8DEab447447e9B58859"
...
describe("SAND BOND", () => {
    it("Should Work", async () => {
        let { user1, user2, D33D, treasury, Bond, SAND, deployer } = await setup()
        let user1Balance = await SAND.balanceOf(user1.address)
        ...
        let user2Balance = await SAND.balanceOf(user2.address)
...
const setup = async () => {
    ...
    const SAND = new ethers.Contract(addresses.tokens.sand, IERC20_ABI, deployer)
    let D33D = await setupD33D(deployer)

    let treasury = await setupTreasury(D33D, deployer)
    let Bond = await setupBond(D33D, treasury, deployer)//await ethers.getContractAt("BondContract", addresses.bond.SANDBond)
    ...
    // await treasury.connect(deployer).toggle(2, SAND.address, ethers.constants.AddressZero)

    await SAND.connect(unlockedSigner).transfer(user1.address, ethers.utils.parseUnits("5", "18"))
    await SAND.connect(unlockedSigner).transfer(user2.address, ethers.utils.parseUnits("500", "18"))
    await SAND.connect(user1).approve(Bond.address, ethers.constants.MaxUint256)
    await SAND.connect(user2).approve(Bond.address, ethers.constants.MaxUint256)

    return { user1, user2, D33D, treasury, Bond, SAND, deployer }
}
...
const setupBond = async (d33d, treasury, deployer) => {
    ...
    let bondContract = await upgrades.deployProxy(BondContract, [d33d.address, addresses.tokens.sand, treasury.address,
    ...
    await bondContract.initializeBondTerms(
        ...
        ethers.utils.parseEther("0.2"), // _minimumPrice //1 d33d = 0.2 sand
